...
$id = $_GET['id'];
$sql="SELECT SupplierName FROM supplier WHERE SupplierID = '$id' ";
...
<head>
    ...
    <title>Form Import Product</title>
    <!-- Bootstrap CSS -->
            ...
            <div class="row">
                ...
                <div class="col-8 mt-2 ">
                    <div class="col text-center">
                        <h1 class="text-dark">
                          Supplier : <?=$id?> - <?= $row['SupplierName']?>
                            <!-- ปุ่มเพิ่ม -->
                        </h1>
                    </div>
                    <div class="bg-white border rounded p-4 shadow">

                        <form action="insert_supplier+product.php" method="post" >
                            <div class="table-responsive">
                                <table id="productTable" class="table caption-top table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <input type="hidden" class="form-control" name="Ordered_by_id"
                                            value=<?php echo $Sellerss;?>>

                                        <input type="hidden" class="form-control" name="OrderDate"
                                            value=<?php echo $OrderDate;?>>


                                        <td>
                                            <select id="ProductID" name="ProductID[]" class="form-select mb-1 mt-1">
                                                <option value="" selected></option>
                                                <?php $sql = "SELECT id_product
                                                                            FROM product_details as p 
                                                                            inner join supplier as s
                                                                            on p.type_code = s.ProductServiceType
                                                                            WHERE SupplierID = '$id'
                                                                            ";
                                                                $hand = mysqli_query($conn,$sql);
                                                                while ($row= mysqli_fetch_array($hand)) {
                                                            ?>
                                                <option value=<?= $row['id_product']?>><?= $row['id_product']?>
                                                </option>
                                                <?php
                                                            }
                                                            mysqli_close($conn);
                                        ...
                                        <td>
                                            <input type="number" id="Quantity" name="Quantity[]"
                                                class="form-control mb-1 mt-1" placeholder="Quantity" min="1" oninput="this.value=this.value.replace(/[^0-9]/g,'')" required>
                                        </td>

                                        <td>
                                            <input type="number" id="UnitPrice" name="UnitPrice[]"
                                                class="form-control mb-1 mt-1" placeholder="UnitPrice" min="1" required oninput="this.value=this.value.replace(/[^0-9]/g,'')">
                                        </td>

                                        <td>
                                            <div class="col text-center">
                                                <button onclick="deleteRow(this)"
                                                    class="btn btn-danger btn-sm mb-1 mt-2 ">Delete row-</button>
                                            </div>
                                        </td>
                                    </tr>
                                </table>
                            ...
                            <div class="d-grid gap-2 col-6 mx-auto">
                                <input class="btn btn-success " type="submit" value="Import Product">
                            </div>
                        </form>
...
function addRow() {
    ...
    cell1.innerHTML = '<select id="ProductID" name="ProductID[]" class="form-select mb-1 mt-1">' +
        '<option selected>-</option>' +
        '<?php include 'condb_store.php';
        ...
        '</select>';
    cell2.innerHTML =
        '<input type="number" id="Quantity" name="Quantity[]" class="form-control mb-1 mt-1" placeholder="Quantity" min="1" required>';
    cell3.innerHTML =
        '<input type="number" id="UnitPrice" name="UnitPrice[]" class="form-control mb-1 mt-1" placeholder="UnitPrice" min="1" required>';
    cell4.innerHTML =
        '<div class="col text-center"><button onclick="deleteRow(this)" class="btn btn-danger btn-sm mb-1 mt-2" >Delete row-</button></div>';
}
